...
const BookHistory = () => {
  ...
  return (
    <div style={{ position: 'relative', minHeight: '100vh' }}>
      ...
      />
      <div className="max-w-screen-lg mx-auto p-6 bg-white shadow-md rounded-md backdrop-filter backdrop-blur-sm bg-opacity-30" style={{ zIndex: 1 , backgroundColor: 'rgba(255, 255, 255, 0.8)' }}>
        <h1 className="text-2xl font-bold mb-4 text-black">Book's List</h1>
        ...
        <div className="flex justify-center"> {/* Centering the table */}
          {books.length > 0 ? (
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody>
                {books.map((book, index) => (
                  <tr key={index}>
                    <td className="py-2 px-4 border border-gray-400 text-black">{book.Book_Id}</td>
                    <td className="py-2 px-4 border border-gray-400 text-black">{book.Book_Title}</td>
                    <td className="py-2 px-4 border border-gray-400 text-black">{book.ROll_No}</td>
                    <td className="py-2 px-4 border border-gray-400 text-black">{formatDate(book.Issue_Date)}</td>
                    <td className="py-2 px-4 border border-gray-400 text-black">{formatDate(book.Excepted_Date)}</td>
                    <td className="py-2 px-4 border border-gray-400 text-black">{formatDate(book.return_Date)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
